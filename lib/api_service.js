...
exports._getCorpToken = function (authCorpId, permanentCode, callback) {
  ...
  const callbackMy = (err, token, res) => {
    if (!err) {
      that.setCorpAccessToken(authCorpId, token);
    }
    callback(err, token, res)
  };
  this.request(url, postJSON(data), wrapper(callbackMy));
};

exports.ensureCorpAccessToken = async (corpId, permanentCode) => {
  // 调用用户传入的获取token的异步方法，获得token之后使用（并缓存它）。
  const token = this.getCorpAccessToken(corpId);
  if (token) {
    return token;
  }

  const getCorpTokenQ = Q.bind(this.getCorpToken, this);
  await getCorpTokenQ(corpId, permanentCode);

  return this.getCorpAccessToken(corpId);
}

exports.setCorpAccessToken = function (corpId, token) {
  const expireTime = Date.now() + (token.expires_in - 10) * 1000;
  token.expires_in = expireTime;
  this.corpAccessToken[corpId + ''] = token;
};

exports.getCorpAccessToken = function (corpId) {
