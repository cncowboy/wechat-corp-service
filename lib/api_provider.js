...
var API = function (corpid, providersecret,getToken, saveToken) {
  ...
  this.store = null;
  this.getToken = getToken || await function () {
    return this.store;
  };
  this.saveToken = saveToken || await function (token) {
    this.store = token;
...
API.prototype.getProviderToken = async function () {
  ...
  var that = this;
  const data = this.request(url, postJSON(data))
  var token = ProviderAccessToken(data);
  await that.saveToken(token);
  return token;
};
 ...
 * @param {Function} callback 回调函数
 */
API.prototype.getLatestToken = async function () {
  var that = this;
  // 调用用户传入的获取token的异步方法，获得token之后使用（并缓存它）。
  const token = await that.getToken();
  // 有token
  if (token) {
    return ProviderAccessToken(token);
  } else {
    return await that.getProviderToken();
  }
};
